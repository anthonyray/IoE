		...
		async.eachSeries(vh.sensors,
			function(thing, callback){
				db.saveValue(thing,function(err){
					if(err){
						console.log("[DB] ERROR WHILE SAVING ",thing.get_hardwareId());
						callback(err);
					}
					else
					{
				...
				if (thing.checkRules) { //
					thing.checkRules(function(ruleTriggered,actuatorId,value){
						console.log("[RULES] Checking for trigger to be triggered");
						if (ruleTriggered){
							console.log("[RULES] Trigger triggered ! Actuator with id :", actuatorId);
