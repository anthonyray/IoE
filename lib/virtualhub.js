...
// Adding a saveValue to the sensors
yoctolib.YTemperature.prototype.setPreviousValue = function(){
  this._previousValue = this._currentValue;
}
yoctolib.YLightSensor.prototype.setPreviousValue = function(){
  this._previousValue = this._currentValue;
}
yoctolib.YHumidity.prototype.setPreviousValue = function(){
  this._previousValue = this._currentValue;
}
yoctolib.YPressure.prototype.setPreviousValue = function(){
  this._previousValue = this._currentValue;
}

// Adding a pushRules for sensors
yoctolib.YTemperature.prototype.pushRule = function(rule){
    this.rules.push(rule);
}

yoctolib.YLightSensor.prototype.pushRule = function(rule){
  this.rules.push(rule);
}

yoctolib.YHumidity.prototype.pushRule = function(rule){
  this.rules.push(rule);
}

yoctolib.YPressure.prototype.pushRule = function(rule){
  this.rules.push(rule);
}
...
yoctolib.YTemperature.prototype.checkRules = function(cb){
  var self = this;
  if (this.rules){
    this.rules.forEach(function(rule){
      self.checkRule(rule,cb);
    });
  }
  else
    cb(false);
...
yoctolib.YTemperature.prototype.checkRule = function(rule,cb){
  // Operator parsing
  if (rule.operator == ">"){
    var threshold = parseInt(rule.threshold);
    if ( (this._currentValue > threshold)  && (this._previousValue < threshold || !this._previousValue) )
      cb(true, rule.actuatorId, rule.value); // Rule is verified and it triggers the action defined in the callback
    else
      cb(false);
  }
...
yoctolib.YLightSensor.prototype.checkRules = function(cb){
  var self = this;
  if (this.rules){
    this.rules.forEach(function(rule){
      self.checkRule(rule,cb);
...
yoctolib.YLightSensor.prototype.checkRule = function(rule,cb){
  // Operator parsing
  if (rule.operator == ">"){
    var threshold = parseInt(rule.threshold);
    if ( (this._currentValue > threshold)  && (this._previousValue < threshold || !this._previousValue) )
...
yoctolib.YHumidity.prototype.checkRules = function(cb){
  var self = this;
  if (this.rules){
    this.rules.forEach(function(rule){
      self.checkRule(rule,cb);
...
yoctolib.YHumidity.prototype.checkRule = function(rule,cb){
  // Operator parsing
  if (rule.operator == ">"){
    var threshold = parseInt(rule.threshold);
    if ( (this._currentValue > threshold)  && (this._previousValue < threshold || !this._previousValue) )
...
yoctolib.YPressure.prototype.checkRules = function(cb){
  var self = this;
  if (this.rules){
    this.rules.forEach(function(rule){
      self.checkRule(rule,cb);
...
yoctolib.YPressure.prototype.checkRule = function(rule,cb){
  // Operator parsing
  if (rule.operator == ">"){
    var threshold = parseInt(rule.threshold);
    if ( (this._currentValue > threshold)  && (this._previousValue < threshold || !this._previousValue) )
...
module.exports.prototype.initRules = function(cb) { // Attach each rules and actions to concerned sensors
...
module.exports.prototype.init = function (cb) {
  ...
  try
  {
    YAPI.RegisterHub(conf.virtualHub.ip+":"+conf.virtualHub.port) // The RegisterHub function does not take the port into account, that's why we don't use it
